...
import InsuranceAPIService from '@/services/insuranceAPI';
import type { Claim, FraudAlert } from '@/types/insurance';
import {
    ChartBarIcon,
    CheckCircleIcon,
    ClockIcon,
...
const ClaimsProcessing: React.FC<ClaimsProcessingProps> = ({ className = '' }) => {
  const { user } = useAuth();
  ...
  const runFraudDetection = async (claimId: string) => {
    try {
      setIsLoading(true);
  ...
  const uploadDocument = async (claimId: string, file: File, type: string) => {
    try {

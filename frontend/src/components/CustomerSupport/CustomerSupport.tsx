...
import type { ChatMessage, ChatSession } from '@/types/insurance';
import {
...
const CustomerSupport: React.FC<CustomerSupportProps> = ({ className = '' }) => {
  ...
  const sendWelcomeMessage = async (category: string) => {
  ...
  const loadPreviousSession = async (session: ChatSession) => {
    try {
      setIsLoading(true);
